...
function normalizePulseStatus(raw?: string): AppStatus {
  ...
  if (s === "down" || s === "error" || s === "unhealthy" || s === "offline")
    return "down";
  return "operational";
}
...
function infrastructureFromList(items: InfraListEntry[]): Infrastructure {
  ...
  const dbCandidates = items.filter((i) => {
    ...
    return (
      id.includes("mongo") || id.includes("postgres") || id.includes("mysql")
    );
  });
...
function infrastructureFromUnknown(infra: unknown): Infrastructure {
  ...
  if (d === "mongodb" || d === "postgres" || d === "unknown")
    partial.database = d;
...
function metricsFromUnknown(
  ...
  if (!isRecord(metrics)) {
    return mergeMetrics(
      opts.dbLatencyMs != null ? { latency_ms: opts.dbLatencyMs } : {},
    );
  }
  ...
  if (typeof technical.latency_ms === "number")
    out.latency_ms = technical.latency_ms;
  if (typeof technical.uptime_percent === "number")
    out.uptime_percent = technical.uptime_percent;
  if (typeof technical.requests_24h === "number")
    out.requests_24h = technical.requests_24h;
  if (typeof technical.error_rate === "number")
    out.error_rate = technical.error_rate;
...
function kpisFromMetricsUnknown(
  metrics: unknown,
  existing: BusinessKPIs[],
): BusinessKPIs[] {
  ...
  if (!isRecord(business) || Object.keys(business).length === 0) {
    ...
    if (
      typeof technical.node_version === "string" &&
      technical.node_version.length > 0
    ) {
...
export function normalizeBackendPulse(
  ...
  const infraList = isInfraComponentList(d.infrastructure)
    ? d.infrastructure
    : null;
  ...
  const kpis = kpisFromMetricsUnknown(
    d.metrics,
    (d.kpis ?? []) as BusinessKPIs[],
  );
