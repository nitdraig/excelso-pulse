...
| `VOICE_WEBHOOK_SECRET` | Opcional: secreto fallback legacy single-user para webhook de Dialogflow. |
| `VOICE_WEBHOOK_ALT_HEADER_NAME` | Opcional: nombre de cabecera donde se envía el mismo secreto (p. ej. `x-monitoring-token`). |
| `VOICE_DEFAULT_USER_EMAIL` | Solo fallback legacy: correo MongoDB si no se envía `x-excelso-user-email`. |
