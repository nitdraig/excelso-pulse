...
2. **Environment**
   ...
   Optional aggregator tuning: `PULSE_FETCH_TIMEOUT_MS`, `PULSE_ROUND_TIMEOUT_MS`, `PULSE_CACHE_TTL_MS`, `PULSE_RATE_LIMIT_*`, `PULSE_SOURCES`, `PULSE_MERGE_ENV_SOURCES`. See [`web-app/docs/pulse-aggregator.md`](web-app/docs/pulse-aggregator.md). Dashboard auto-refresh (client): `NEXT_PUBLIC_DASHBOARD_AUTO_REFRESH_MS` (default `30000`; `0` disables background polls). Manual **Refresh** calls `/api/portfolio?refresh=1` to bypass the aggregator cache once. **Telegram bot (multi-tenant):** `TELEGRAM_BOT_TOKEN`, `TELEGRAM_WEBHOOK_SECRET`, `TELEGRAM_BOT_USERNAME`; webhook `POST /api/v1/telegram/webhook`. See [`web-app/docs/telegram-bot.md`](web-app/docs/telegram-bot.md).
...
3. **Run locally**
   ...
   Open [https://pulse.excelso.xyz](https://pulse.excelso.xyz).
...
- [`docs/estrategia-voz-hub.md`](docs/estrategia-voz-hub.md) — Definitive **voice strategy** (Hub vs npm package, token security, roadmap, TTS, channels: Dialogflow, HA, Google Home). *Spanish.*
...
- [`web-app/docs/voice-fulfillment.md`](web-app/docs/voice-fulfillment.md) — Dialogflow ES fulfillment, JSON report endpoint, curl examples, and `pulse_contract` fields.
